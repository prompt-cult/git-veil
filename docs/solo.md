...
`target/release/git-veil`) and have `gpg` available. git-veil does **not**
...
and `me.pub` (public — what `trust` pins). git-veil has **no** key
...
your key is imported (step 2), `git-veil export example@github.com
...
the per-machine key store `$HOME/.git-veil/secret-keys.pgp`, never committed
...
**pinned on this machine** in `$HOME/.git-veil` — outside the repository,
...
```sh
$ git-veil tell example@github.com me.pub
✓ Added example@github.com to keyring
```

What just happened: `hide` encrypts to every key in the signed keyring, so
even solo you must be in it. The keyring at `.git-veil/keyring` is signed by
...
```sh
$ git-veil unhide .env          # decrypt one file in place, delete its ciphertext
... edit .env ...
...
$ git-veil hide                 # re-encrypt, delete the plaintext
...
without disturbing anything, `git-veil cat .env` writes the plaintext to
...
`--passphrase-stdin` (exactly one line on stdin). `git-veil help <command>`
