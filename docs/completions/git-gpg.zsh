...
'init:Initialize git-gpg in the current repository' \
'import:Import your private key(s) into the git-gpg key store' \
'trust:Establish trust for a repository by verifying the owner'\''s signing key' \
'tell:Add a collaborator'\''s public key to the keyring' \
'removeperson:Remove a collaborator from the keyring' \
'add:Add a file to be encrypted' \
'remove:Remove a file from encryption tracking' \
'list:List all tracked files' \
'hide:Hide - encrypt all tracked files' \
'reveal:Reveal - decrypt all tracked files' \
'cat:Cat - decrypt a single tracked file to stdout' \
'unhide:Unhide - decrypt a single tracked file back to plaintext and delete its ciphertext' \
'changes:Changes - report where plaintext differs from the last hidden version' \
'show-repo-id:Show the repository ID' \
'whoami:Show your identity' \
'verify-keyring:Verify the keyring signature' \
'list-keys:List all keys in the keyring (requires a verified keyring signature)' \
'clean:Clean - remove all git-gpg metadata' \
